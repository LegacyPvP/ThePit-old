        ...
        if ($this->testPermissionSilent($sender)) {
            try {
                if (isset($args[0], $args[1], $args[2])) {
                    ...
                    $amount = is_numeric($args[1]) ? (int)$args[1] : throw new LanguageException("messages.commands.remove.invalid-amount", ["{amount}" => $args[1]], ServerUtils::PREFIX_2);
                    $currency = match ($args[2]) {
                        ...
                        default => throw new LanguageException("messages.commands.remove.invalid-currency", ["{currency}" => $args[2]], ServerUtils::PREFIX_2)
                    };
                    if ($target instanceof LegacyPlayer) {
                        $target->getCurrencyProvider()->remove($currency, $amount);
